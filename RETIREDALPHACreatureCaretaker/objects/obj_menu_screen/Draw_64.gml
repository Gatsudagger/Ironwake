...
if (active_tab == 0) {
    ...
    if (is_struct(_gc.starter_creature)) {
        ...
        // Bond display
        var _bond       = _c.bond;
        var _bond_label = scr_get_bond_label(_bond);
        draw_set_halign(fa_center);
        draw_set_colour(make_colour_rgb(255, 180, 100));
        draw_text_transformed(_cx, _bar_y + 8*_bar_gap + 80,
            "Bond: " + _bond_label + " (" + string(_bond) + "/50)", 1.0, 1.0, 0);
        draw_set_colour(c_dkgray);
        draw_rectangle(_bar_x+35, _bar_y + 8*_bar_gap + 100,
            _bar_x+35+_bar_w, _bar_y + 8*_bar_gap + 114, false);
        draw_set_colour(make_colour_rgb(255, 140, 60));
        draw_rectangle(_bar_x+35, _bar_y + 8*_bar_gap + 100,
            _bar_x+35+round(_bar_w * (_bond / 50)), _bar_y + 8*_bar_gap + 114, false);
    } else {
