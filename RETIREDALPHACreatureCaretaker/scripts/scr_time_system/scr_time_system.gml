...
function scr_time_creature_encounters(creature_id) {
	var m = global.minutes_in_day;
	switch (creature_id) {
		case SPECIES.HAREHOUND: return (m >= 360  && m < 720);
		case SPECIES.AMPHIBI:   return (m >= 720  && m < 1080);
		case SPECIES.SALAPENT:  return (m >= 1080 && m < 1440);
		case SPECIES.RAPTOWL:   return (m < 360);
		case SPECIES.BOULDEER:
