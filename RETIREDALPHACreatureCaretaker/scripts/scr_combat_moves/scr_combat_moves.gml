...
/// @desc Returns the stamina cost to fire a move.
/// @param {Struct} move   A move struct from global.combat_moves
/// @returns {real}
function scr_get_move_stamina_cost(move) {
	if (move.type == MOVE_TYPE.BUFF || move.type == MOVE_TYPE.DEBUFF) return 6;
	if (move.type == MOVE_TYPE.PROJECTILE) return 4;
	if (move.dash_speed > 0 || move.base_damage >= 18) return 8;
	if (move.cooldown_secs <= 50) return 3;
	return 5;
}
