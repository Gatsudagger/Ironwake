...
**Also DONE 2026-07-04:** Curse → Hexed (kind `hexed`: keeps +4 dmg-taken, detonation bonuses ×2 on the bearer — burn +80% crit, root/frost +60%, weaken +30%, Exposed +24, bleed +10/tick, void 60% lifesteal, shock arc 66%, Mortality capped 80% — each detonation spreads a non-stacking +2 dmg-taken "Hex Spread" (2 turns) to all other enemies); Lucky Find = 20% no-consume on use (drop bonus removed); Shadow Meld = dodge primes a guaranteed crit on the next damaging attack (old +15 dodge gone); Treasure Hunter VERIFIED not a no-op (rooms were 40% one item) → now +1 guaranteed item on top of the 40% roll, popup shows the bonus line.
**Still open:** nothing — enemy intent SHIPPED (ce63ef9), expression picks SHIPPED (ce63ef9), flavor pass DONE 2026-07-05: all 49 desc_short/desc_full rewritten (evocative action line + line-broken "- " technical breakdown; school words Capitalized; "elemental" → resolved school Arcane/Fire; stale text fixed — Entropy poison→Void DoT, Curse desc now discloses Hexed ×2 detonations + Hex Spread, Flurry's 3-strike identity disclosed). §7 cast barks / separate lore-line field deliberately skipped — M chose in-desc flavor instead.
